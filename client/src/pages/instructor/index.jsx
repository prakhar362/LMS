    ...
    <div className="flex h-full min-h-screen bg-slate-50">
      <aside className="w-72 bg-white border-r border-slate-200 hidden md:flex flex-col">
        <div className="p-8 border-b border-slate-50 flex items-center space-x-3">
          <div className="h-10 w-10 bg-indigo-600 rounded-2xl flex items-center justify-center shadow-indigo-200 shadow-xl">
             <GraduationCap className="h-6 w-6 text-white" />
          </div>
          <span className="text-xl font-black text-slate-800 tracking-tight font-outfit uppercase">Academy OS</span>
        </div>
             ...
             <nav className="space-y-1">
               {menuItems.map((menuItem) => (
                 <button
                   ...
                   className={`w-full flex items-center space-x-4 px-6 py-4 rounded-3xl text-sm font-black transition-all group ${
                     activeTab === menuItem.value 
                       ? "bg-indigo-50 text-indigo-600 shadow-sm ring-1 ring-indigo-100" 
                       : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"
                   }`}
                 >
                   <menuItem.icon className={`h-5 w-5 transition-colors ${
                     activeTab === menuItem.value ? "text-indigo-600" : "text-slate-300 group-hover:text-slate-500"
                   }`} />
          ...
          <div>
             ...
             <nav className="space-y-1">
                ...
                <button 
                   ...
                   className="w-full flex items-center space-x-4 px-6 py-4 rounded-3xl text-sm font-black text-rose-500 hover:bg-rose-50 transition-all group"
                >
                   <LogOut className="h-5 w-5 text-rose-300 group-hover:text-rose-500" />
      ...
      <main className="flex-1 h-screen overflow-hidden flex flex-col">
        <header className="h-20 bg-white border-b border-slate-200 px-10 flex items-center justify-between z-10">
           <div className="flex items-center space-x-3">
              ...
              <span className="text-[10px] font-black uppercase text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">Strat-A-Module (Primary)</span>
           </div>
